...
public class AdaptorCountry extends RecyclerView.Adapter<AdaptorCountry.ViewHolder> {
    ...
    public AdaptorCountry(List<Country> mdata, Context context) {
        this.mdata = mdata;
        mcontext = context;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (MyApplication.getInstance().getPrefManager().getLang().equals("1")) {
            holder.name.setText(mdata.get(position).getCountry().getNameAr());
        } else {
            holder.name.setText(mdata.get(position).getCountry().getNameEn());
        }
