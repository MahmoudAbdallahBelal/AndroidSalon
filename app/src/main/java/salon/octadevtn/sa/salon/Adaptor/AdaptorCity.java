...
public class AdaptorCity extends RecyclerView.Adapter<AdaptorCity.ViewHolder> {
    ...
    public AdaptorCity(List<City> mdata, Context context) {
        this.mdata = mdata;
        mcontext = context;
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (MyApplication.getInstance().getPrefManager().getLang().equals("1")) {
            holder.name.setText(mdata.get(position).getNameAr());
        } else {
            holder.name.setText(mdata.get(position).getNameEn());
        }
